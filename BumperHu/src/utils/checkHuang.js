...
var CheckHuang = function(laizi,hhType){
    this.laizi = laizi;
    this.hhType = hhType;
};
//console.log = function(){}
var max = 4;
var pro = CheckHuang.prototype ;
...
pro.checkHu = function(user,pai){
    ...
    var huType = [];
    var isHu = this.checkUnHasJiang(user,pai);
    if(isHu){
    ...
    if(isHu){
        huType.push(isHu);
        var isqinyise = this.qingyise(user,pai);
        if(isqinyise){
            huType.push(isqinyise);
        }
    }
    if(huType.length > 1 && huType.indexOf(1) != -1){
        var index = huType.indexOf(1);
...
pro.qidui = function(user,pai){
    ...
    var allPai = this.qiduiQransform(mahjongs);
    var duizi = 0;
    var isHaoqi = 0;
    var isThree = 0;
    ...
    var hasLaizi = 7 - duizi;
    if(hasLaizi <= laiziCount){
        var lastOne = mahjongs[mahjongs.length - 1];
...
pro.playToTing = function(user){
    var arr = [];
    console.log("playToTing user ",user);
